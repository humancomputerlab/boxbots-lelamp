...
```sh
git clone https://github.com/humancomputerlab/boxbots_lelampruntime/tree/Master
cd boxbots_lelampruntime
```
> If you are working on a 12V Lelamp kit then switch to the 12Vruntime branch on this repo. If you are running the 7.4V kit then remain on master.

```sh
git checkout 12Vruntime
```
...
> Note: If your motors were previously configured to use 5V then you will receive a voltage error when running through these steps. Notify us if that is the case.

<!-- - Connect 5V power, run motor setup. You will receive a voltage error.
- Then plug in the 12V Power and run the setup again. This time the motor ID will be set.
- The setup starts at ID 5. To setup the other IDs, keep the motor plugged in a press enter until it sets the correct ID -->
